...
export function RoomJoinCard({ onCreateRoom, onJoinRoom, isConnecting, initialRoomId = '' }: Props) {
  ...
  const handleJoin = () => {
    ...
    localStorage.setItem('fd_name', name);
    onJoinRoom(roomId.toUpperCase(), name);
  };
          ...
          {mode === 'initial' ? (
            <div className="grid grid-cols-2 gap-4">
              <button
                ...
                className="flex flex-col items-center justify-center gap-3 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 disabled:cursor-not-allowed text-white p-6 rounded-2xl transition-all group"
              >
                <Plus className="w-8 h-8 group-hover:scale-110 transition-transform" />
                <span className="font-semibold">建立房間</span>
              </button>
              <button
                onClick={() => setMode('join')}
                disabled={!name}
                className="flex flex-col items-center justify-center gap-3 bg-slate-800 hover:bg-slate-700 disabled:opacity-50 text-white p-6 rounded-2xl transition-all group"
              >
                <LogIn className="w-8 h-8 group-hover:scale-110 transition-transform" />
                <span className="font-semibold">加入房間</span>
              </button>
            </div>
          ) : (
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4">
              <div>
                ...
                <input
                  ...
                  placeholder="輸入 5 位代碼"
                  maxLength={5}
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-white text-center text-2xl font-mono tracking-widest focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all uppercase"
                />
              </div>
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="flex-[2] bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white py-3 rounded-xl font-semibold transition-all"
                >
                  確認加入
                </button>
              </div>
